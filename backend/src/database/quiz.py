...
def get_quiz_words(db: Session, clerk_id: str):
    ...
    try:
        words = db.query(models.Words).filter(models.Words.added_by_user_id == clerk_id, (models.Words.vocabulary == "learning") | (models.Words.vocabulary == "unknown")).order_by(models.Words.time_to_reapet).all()
