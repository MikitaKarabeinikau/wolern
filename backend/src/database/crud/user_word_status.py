...
def create_user_word_status(db: Session, vocabulary_word_id: int) -> models.UserWordStatus:
    ...
    try:
        new_status = models.UserWordStatus(
        ...
        logger.error(f"Error creating UserWordStatus for vocabulary_word_id '{vocabulary_word_id}': {e}", exc_info=True)
        db.rollback()
        raise
...
def get_user_word_status(db: Session, vocabulary_word_id: int) -> models.UserWordStatus:
    ...
    try:
        status = db.query(models.UserWordStatus).filter(
            models.UserWordStatus.vocabulary_word_id == vocabulary_word_id
        ).one()
        return status
        ...
        logger.error(f"Error retrieving UserWordStatus for vocabulary_word_id '{vocabulary_word_id}': {e}", exc_info=True)
        raise

def delete_user_word_status(db:Session, user_word_status_id:int) -> bool:
    """Delete UserWordStatus by its ID."""
    try:
        status = db.query(models.UserWordStatus).filter(models.UserWordStatus.id == user_word_status_id).one()
        db.delete(status)
        db.commit()
        return True
    except NoResultFound:
        ...
        logger.error(f"Error deleting UserWordStatus with id '{user_word_status_id}': {e}", exc_info=True)
        db.rollback()
        raise