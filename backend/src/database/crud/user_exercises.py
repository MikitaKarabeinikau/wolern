...
def create_user_exercise(db:Session, user_id:int, exercise_id:int, word_id:int) -> models.UserExercises:
    ...
    try:
        ...
        existing_exercise = db.query(models.UserExercises).filter(models.UserExercises.user_id == user_id, models.UserExercises.exercise_id == exercise_id, models.UserExercises.word_id == word_id).first()
        if existing_exercise:
            logger.warning(f"User exercise for user_id '{user_id}', exercise_id '{exercise_id}' already exists.")
        ...
        logger.info(f"Successfully created user exercise for user_id '{user_id}', exercise_id '{exercise_id}'.")
        db.commit()
        db.refresh(db_user_exercise)

        return db_user_exercise
    except Exception as e:
        logger.error(f"Error creating user exercise for user_id '{user_id}', exercise_id '{exercise_id}': {e}", exc_info=True)
        db.rollback()
        raise
...
def get_user_exercises_by_user_id(db:Session, user_id:int) -> list[models.UserExercises]:
    ...
    try:
        user_exercises = db.query(models.UserExercises).filter(models.UserExercises.user_id == user_id).all()
