...
def create_user_definitions(db:Session, user_word_status_id:int, part_of_speech:str, definition:str) -> models.UserDefinitions:
    ...
    try:
        ...
        existing_definition = db.query(models.UserDefinitions).filter(models.UserDefinitions.user_word_status_id == user_word_status_id, models.UserDefinitions.part_of_speech == part_of_speech, models.UserDefinitions.definition == definition).first()
        if existing_definition:
            logger.warning(f"Definition '{definition}' for user_word_status_id '{user_word_status_id}' and part_of_speech '{part_of_speech}' already exists.")
        ...
        logger.info(f"Successfully created user definition for user_word_status_id '{user_word_status_id}'.")

        return db_user_definition
    except Exception as e:
        logger.error(f"Error creating user definition for user_word_status_id '{user_word_status_id}': {e}", exc_info=True)
        db.rollback()
        raise

def get_user_definitions_by_user_word_status_id(db:Session, user_word_status_id:int) -> list[models.UserDefinitions]:
    """Get all user definitions for a specific user word status by its ID."""
    try:
        user_definitions = db.query(models.UserDefinitions).filter(models.UserDefinitions.user_word_status_id == user_word_status_id).all()
        logger.info(f"Retrieved {len(user_definitions)} user definitions for user_word_status_id '{user_word_status_id}'.")
        return user_definitions
    except Exception as e:
        logger.error(f"Error getting user definitions for user_word_status_id '{user_word_status_id}': {e}", exc_info=True)
        raise
    
def get_user_definition_by_id(db:Session, id:int) -> models.UserDefinitions:
    """Get a user definition by its ID."""
    try:
        user_definition = db.query(models.UserDefinitions).filter(models.UserDefinitions.id == id).one()
