...
def create_user_example(
    db: Session, user_word_status_id: int, part_of_speech: str, example: str
) -> models.UserExamples:
    ...
    try:
        ...
        existing_example = (
            db.query(models.UserExamples)
            .filter(
                models.UserExamples.user_word_status_id == user_word_status_id,
                models.UserExamples.part_of_speech == part_of_speech,
                models.UserExamples.example == example,
            )
            .first()
        )
        if existing_example:
            logger.warning(
                f"Example '{example}' for user_word_status_id '{user_word_status_id}' \\\
        ...
        logger.info(
            f"Successfully created user example for user_word_status_id '{user_word_status_id}'."
        )

        return db_user_example
    except Exception as e:
        logger.error(
            f"Error creating user example for user_word_status_id '{user_word_status_id}': {e}",
            exc_info=True,
        )
        db.rollback()
        raise
...
def get_user_examples_by_word_status_id(
    ...
    try:
        user_examples = (
            db.query(models.UserExamples)
            .filter(models.UserExamples.user_word_status_id == user_word_status_id)
            .all()
        )
        logger.info(
            f"Retrieved {len(user_examples)} user examples \\\
                for user_word_status_id '{user_word_status_id}'."
        )
        return user_examples
    except Exception as e:
        logger.error(
            f"Error getting user examples for user_word_status_id '{user_word_status_id}': {e}",
            exc_info=True,
        )
        raise


def get_user_examples_by_part_of_speech(
    db: Session, part_of_speech: str
) -> list[models.UserExamples]:
    """Get all user examples for a specific definition by its ID."""
    try:
        user_examples = (
            db.query(models.UserExamples)
            .filter(models.UserExamples.part_of_speech == part_of_speech)
            .all()
        )
        logger.info(
            f"Retrieved {len(user_examples)} user examples for part_of_speech '{part_of_speech}'."
        )
        return user_examples
    except Exception as e:
        logger.error(
            f"Error getting user examples for part_of_speech '{part_of_speech}': {e}", exc_info=True
        )
        raise
