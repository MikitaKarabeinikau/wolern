...
def create_user_exercise_progress(db: Session, id: int) -> models.UserExerciseProgress:
    ...
    try:
        exercise = (
            db.query(models.UserExerciseProgress)
            .filter(models.UserExerciseProgress.id == id)
            .first()
        )
        if exercise:
        ...
        logger.error(
            f"Error creating user exercise progress for user_exercise_id '{id}': {e}", exc_info=True
        )
        db.rollback()
        raise
...
def get_user_exercise_progress_by_id(db: Session, id: int) -> models.UserExerciseProgress:
    ...
    try:
        user_exercise_progress = (
            db.query(models.UserExerciseProgress).filter(models.UserExerciseProgress.id == id).one()
        )
...
def increase_correct_count(db: Session, id: int) -> models.UserExerciseProgress:
    ...
    try:
        ...
        user_exercise_progress.last_attempted = datetime.now(
            timezone.utc
        ...
        logger.info(
            f"Correct count for user exercise progress with id '{id}' increased successfully."
        )
        ...
        logger.error(
            f"Error increasing correct count for user exercise progress with id '{id}': {e}",
            exc_info=True,
        )
        db.rollback()
        raise
...
def increase_incorrect(db: Session, id: int) -> models.UserExerciseProgress:
    ...
    try:
        ...
        logger.info(
            f"Incorrect count for user exercise progress with id '{id}' increased successfully."
        )
        ...
        logger.error(
            f"Error increasing incorrect count for user exercise progress with id '{id}': {e}",
            exc_info=True,
        )
        db.rollback()
        raise
