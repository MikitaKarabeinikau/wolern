...
def create_warning(db: Session, word_id: int, warning: str) -> models.Warnings:
    ...
    try:
        ...
        new_warning = models.Warnings(word_id=word_id, warning_message=warning)
        db.add(new_warning)
        db.commit()
...
def get_warning_by_id(db: Session, warning_id: int) -> Optional[models.Warnings]:
    ...
    try:
        return db.query(models.Warnings).filter(models.Warnings.id == warning_id).first()
    except Exception as e:
...
def get_word_warnings(db: Session, word_id: int) -> List[models.Warnings]:
    ...
    try:
        return db.query(models.Warnings).filter(models.Warnings.word_id == word_id).all()
    except Exception as e:
...
def count_word_warnings(db: Session, word_id: int) -> int:
    ...
    try:
        return db.query(models.Warnings).filter(models.Warnings.word_id == word_id).count()
    except Exception as e:
