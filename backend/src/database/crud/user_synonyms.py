...
def create_user_synonym(db:Session, user_word_status_id:int, synonym:str) -> models.UserSynonyms:
    ...
    try:
        ...
        existing_synonym = db.query(models.UserSynonyms).filter(models.UserSynonyms.user_word_status_id == user_word_status_id, models.UserSynonyms.synonym == synonym).first()
        if existing_synonym:
            logger.warning(f"Synonym '{synonym}' for UserWordStatus ID '{user_word_status_id}' already exists.")
            return existing_synonym

        new_synonym = models.UserSynonyms(
        ...
        logger.error(f"Error creating UserSynonyms for UserWordStatus ID '{user_word_status_id}': {e}", exc_info=True)
        db.rollback()
        raise
...
def get_user_synonyms(db:Session, user_word_status_id:int) -> list[models.UserSynonyms]:
    ...
    try:
        synonym = db.query(models.UserSynonyms).filter(models.UserSynonyms.user_word_status_id == user_word_status_id).all()
        return synonym
    except Exception as e:
        logger.error(f"Error retrieving UserSynonyms for UserWordStatus ID '{user_word_status_id}': {e}", exc_info=True)
        raise

def get_user_synonym_by_id(db:Session, user_synonym_id:int) -> models.UserSynonyms:
    """Retrieve a UserSynonyms by its ID."""
    try:
        synonym = db.query(models.UserSynonyms).filter(models.UserSynonyms.id == user_synonym_id).one()
        return synonym
    except NoResultFound:
        logger.warning(f"User synonym with id '{user_synonym_id}' not found.")
        return None
    except Exception as e:
        logger.error(f"Error retrieving UserSynonyms with id '{user_synonym_id}': {e}", exc_info=True)
        raise
...
def update_user_synonym(db:Session, user_synonym_id:int, new_synonym:str) -> models.UserSynonyms:
    ...
    try:
        synonym_entry = db.query(models.UserSynonyms).filter(models.UserSynonyms.id == user_synonym_id).one()
