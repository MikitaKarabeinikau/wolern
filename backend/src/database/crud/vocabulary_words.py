...
def create_vocabulary_word(db: Session, vocabulary_id: int, word_id: int) -> models.VocabularyWords:
    ...
    try:
        ...
        existing = db.query(models.VocabularyWords).filter(
        ...
        # Create entry
        new_word = models.VocabularyWords(
...
def get_vocabulary_words(db: Session, vocabulary_id: int, skip: int = 0, limit: int = 100) -> List[models.VocabularyWords]:
    """Get words in a vocabulary with pagination."""
    try:
        return db.query(models.VocabularyWords).filter(
            models.VocabularyWords.vocabulary_id == vocabulary_id
...
def delete_vocabulary_word(db: Session, vocabulary_word_id: int) -> bool:
    ...
    try:
        word_entry = db.query(models.VocabularyWords).filter(
            models.VocabularyWords.id == vocabulary_word_id
        ).first()
...
def remove_word_from_vocabulary(db: Session, vocabulary_id: int, word_id: int) -> bool:
    ...
    try:
        entry = db.query(models.VocabularyWords).filter(
            models.VocabularyWords.vocabulary_id == vocabulary_id,
            models.VocabularyWords.word_id == word_id
        ).first()
