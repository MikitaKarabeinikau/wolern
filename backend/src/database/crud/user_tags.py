...
def create_user_tag(db:Session, user_word_status_id:int, tag:str) -> models.UserTags:
    ...
    try:
        ...
        existing_tag = db.query(models.UserTags).filter(models.UserTags.user_word_status_id == user_word_status_id, models.UserTags.tag == tag).first()
        if existing_tag:
            logger.warning(f"Tag '{tag}' for user_word_status_id '{user_word_status_id}' already exists.")
            return existing_tag

        new_tag = models.UserTags(
