...
def create_user_quiz_progress(db: Session, user_word_status_id: int) -> models.UserQuizProgress:
    ...
    try:
        new_progress = models.UserQuizProgress(
            user_word_status_id=user_word_status_id, learning_stage=1  # Initial learning stage
        )
        db.add(new_progress)
        db.commit()
        ...
        logger.error(
            f"Error creating UserQuizProgress for UserWordStatus ID '{user_word_status_id}': {e}",
            exc_info=True,
        )
        db.rollback()
        raise
...
def get_user_quiz_progress(db: Session, user_word_status_id: int) -> models.UserQuizProgress:
    ...
    try:
        ...
        logger.warning(
            f"User quiz progress for UserWordStatus ID '{user_word_status_id}' not found."
        )
        return None
    except Exception as e:
        logger.error(
            f"Error retrieving UserQuizProgress for UserWordStatus ID '{user_word_status_id}': {e}",
            exc_info=True,
        )
        raise
...
def delete_user_quiz_progress(db: Session, user_quiz_progress_id: int) -> bool:
    ...
    try:
        ...
        logger.warning(
            f"User quiz progress with id '{user_quiz_progress_id}' not found for deletion."
        )
        return False
    except Exception as e:
        logger.error(
            f"Error deleting UserQuizProgress with id '{user_quiz_progress_id}': {e}", exc_info=True
        )
        db.rollback()
        raise
