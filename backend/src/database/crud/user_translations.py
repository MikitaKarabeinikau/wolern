...
def create_user_translation(
    db: Session, user_word_status_id: int, language: str, translation: str
    ...
    try:
        ...
        if existing_translation:
            logger.warning(
                f"User translation for UserWordStatus ID '{user_word_status_id}' already exists."
            )
            return existing_translation
        new_translation = models.UserTranslations(
            user_word_status_id=user_word_status_id, language=language, translation=translation
        )
        ...
        logger.error(
            f"Error creating UserTranslations for UserWordStatus ID '{user_word_status_id}': {e}",
            exc_info=True,
        )
        db.rollback()
        raise
...
def update_user_translation(
    ...
    try:
        ...
        logger.error(
            f"Error updating UserTranslations with id '{user_translation_id}': {e}", exc_info=True
        )
        db.rollback()
        raise
...
def delete_user_translation(db: Session, user_translation_id: int) -> bool:
    ...
    try:
        ...
        logger.error(
            f"Error deleting UserTranslations with id '{user_translation_id}': {e}", exc_info=True
        )
        db.rollback()
        raise
