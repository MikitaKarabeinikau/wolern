...
def create_definition(db: Session, word_id: int, part_of_speech: str, definition: str) -> models.Definitions:
    ...
    try:
        ...
        existing_definition = db.query(models.Definitions).filter(
        ...
        if existing_definition:
            logger.info(f"Definition already exists for word_id '{word_id}', part_of_speech '{part_of_speech}'.")
...
def get_definitions_by_word_id(db: Session, word_id: int) -> list[models.Definitions]:
    ...
    try:
        definitions = db.query(models.Definitions).filter(models.Definitions.word_id == word_id).all()
...
def get_definition_by_id(db: Session, definition_id: int) -> models.Definitions:
    ...
    try:
        definition = db.query(models.Definitions).filter(models.Definitions.id == definition_id).one()
