...
def create_multiple_choice_exercise(
    ...
    if not exercise:
        logger.error(
            f"Exercise with ID {exercise_id} does not exist. \\\
                Cannot create multiple-choice exercise."
        )
        db.rollback()
        return None
    ...
    if correct_answer not in options_json:
        logger.error(
            f"Correct answer '{correct_answer}' is not in the provided options.\\\
                 Cannot create multiple-choice exercise."
        )
        db.rollback()
        return None
    ...
    existing = (
        db.query(models.MultipleChoiceExercise)
        .filter(models.MultipleChoiceExercise.exercise_id == exercise_id)
        .first()
    )
    if existing:
    ...
    mc_exercise = models.MultipleChoiceExercise(
        exercise_id=exercise_id, options=options, correct_answer=correct_answer
    )
    db.add(mc_exercise)
    db.commit()
...
def get_multiple_choice_exercise_by_id(db: Session, mc_exercise_id: int):
    ...
    mc_exercise = (
        db.query(models.MultipleChoiceExercise)
        .filter(models.MultipleChoiceExercise.id == mc_exercise_id)
        .first()
    )
