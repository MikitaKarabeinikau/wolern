...
def create_user(
    ...
    try:
        ...
        logger.info(f"Successfully created user with clerk_id '{clerk_id}' and username '{username}'.")
...
def get_user_by_clerk_id(db: Session, clerk_id: str) -> Optional[models.Users]:
    ...
    try:
        user = db.query(models.Users).filter(
            models.Users.clerk_id == clerk_id
        ).first()
...
def update_native_language(db: Session, clerk_id: str, new_native_language: str) -> Optional[models.Users]:
    ...
    try:
        ...
        db.commit()
        db.refresh(user)
        logger.info(f"Native language updated to '{new_native_language}' for user with clerk_id '{clerk_id}'.")
        return user
...
def update_preferred_language(db: Session, clerk_id: str, new_preferred_language: str) -> Optional[models.Users]:
    ...
    try:
        ...
        db.commit()
        db.refresh(user)
        logger.info(f"Preferred language updated to '{new_preferred_language}' for user with clerk_id '{clerk_id}'.")
        return user
