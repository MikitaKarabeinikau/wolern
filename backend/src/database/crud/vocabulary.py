...
def create_vocabulary(db: Session, user_id: int, name: str) -> models.Vocabulary:
    ...
    try:
        if get_number_of_vocabularies_by_user(db, user_id) >= settings.MAX_VOCABULARIES_PER_USER:
            raise ValueError(
                f"User with id '{user_id}' has reached the maximum number of vocabularies."
            )

        db_vocabulary = models.Vocabulary(user_id=user_id, name=name)
        ...
        logger.error(
            f"Error creating vocabulary '{name}' for user_id '{user_id}': {e}", exc_info=True
        )
        db.rollback()
        raise

def get_user_by_vocabulary_id(db: Session, vocabulary_id: int) -> Optional[models.Users]:
    """Get the user who owns a specific vocabulary by vocabulary ID."""
    try:
        vocabulary = (
        ...
        user = (
            ...
            .filter(models.Users.id == vocabulary.user_id)
            .first()
        )
        if user:
            logger.info(f"User with id '{user.id}' found for vocabulary_id '{vocabulary_id}'.")
        else:
            logger.warning(f"User for vocabulary_id '{vocabulary_id}' not found.")
        return user
    except Exception as e:
        logger.error(
            f"Error getting user for vocabulary_id '{vocabulary_id}': {e}", exc_info=True
        )
        raise

def get_vocabulary_by_vocabulary_id(
    db: Session, user_id: int, vocabulary_id: int
) -> Optional[models.Vocabulary]:
...
def get_vocabulary_by_name(
    db: Session, user_id: int, vocabulary_name: str
) -> Optional[models.Vocabulary]:
    """Get vocabulary by name for a specific user."""
    try:
        return (
            db.query(models.Vocabulary)
            .filter(models.Vocabulary.user_id == user_id, models.Vocabulary.name == vocabulary_name)
            .first()
        )
    except Exception as e:
        logger.error(
            f"Error getting vocabulary by name '{vocabulary_name}' for user_id '{user_id}': {e}",
            exc_info=True,
        )
        raise
...
def get_number_of_vocabularies_by_user(db: Session, user_id: int) -> int:
    ...
    try:
        ...
        logger.error(
            f"Error getting number of vocabularies for user_id '{user_id}': {e}", exc_info=True
        )
        raise


def update_vocabulary_name(
    db: Session, user_id: int, vocabulary_id: int, new_name: str
) -> Optional[models.Vocabulary]:
...
def get_vocabularies_by_user(db: Session, user_id: int) -> List[models.Vocabulary]:
    ...
    try:
        vocabularies = (
            db.query(models.Vocabulary).filter(models.Vocabulary.user_id == user_id).all()
        )
...
def delete_vocabulary_by_name(db: Session, user_id: int, vocabulary_name: str) -> bool:
    ...
    try:
        vocabulary = (
            db.query(models.Vocabulary)
            .filter(models.Vocabulary.name == vocabulary_name, models.Vocabulary.user_id == user_id)
            .first()
        )
        if not vocabulary:
            logger.warning(
                f"Vocabulary with name '{vocabulary_name}' not found for user_id '{user_id}'."
            )
            return False
        ...
        logger.error(
            f"Error deleting vocabulary '{vocabulary_name}' for user_id '{user_id}': {e}",
            exc_info=True,
        )
        db.rollback()
        raise
