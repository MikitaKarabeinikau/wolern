...
def create_example(db: Session, word_id: int, part_of_speech: str, example: str) -> models.Examples:
    ...
    try:
        ...
        if existing_example:
            logger.info(
                f"Example already exists for word_id '{word_id}', \\\
                    part_of_speech '{part_of_speech}'."
            )
            return existing_example

        db_example = models.Examples(
            word_id=word_id, part_of_speech=part_of_speech, example=example
        )
        db.add(db_example)
        db.commit()
        ...
        logger.error(
            f"Error creating example for part_of_speech '{part_of_speech}': {e}", exc_info=True
        )
        db.rollback()
        raise
...
def get_examples_by_word_id(db: Session, word_id: int) -> list[models.Examples]:
    ...
    try:
        examples = db.query(models.Examples).filter(models.Examples.word_id == word_id).all()
...
def get_examples_by_part_of_speech(db: Session, part_of_speech: str) -> list[models.Examples]:
    ...
    try:
        examples = (
            db.query(models.Examples).filter(models.Examples.part_of_speech == part_of_speech).all()
        )
        logger.info(f"Retrieved {len(examples)} examples for part_of_speech '{part_of_speech}'.")
        return examples
    except Exception as e:
        logger.error(
            f"Error getting examples for part_of_speech '{part_of_speech}': {e}", exc_info=True
        )
        raise
