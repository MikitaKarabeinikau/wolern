...
async def create_vocabulary(
    ...
    try:
        ...
        vocabulary = vocab_crud.create_vocabulary(
            db=db, user_id=user["id"], name=vocabulary_data.name  # ✅ From authenticated user
        )
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="Failed to create vocabulary"
        )
...
async def get_vocabulary_by_id(
    vocabulary_id: int, user: dict = Depends(get_current_user), db: Session = Depends(get_db)
):
    """
    ...
    try:
        vocabulary = vocab_crud.get_vocabulary_by_vocabulary_id(
            db=db, user_id=user["id"], vocabulary_id=vocabulary_id
        )
...
async def update_vocabulary_name(
    ...
    try:
        vocabulary = vocab_crud.update_vocabulary_name(
            db=db, user_id=user["id"], vocabulary_id=vocabulary_id, new_name=vocabulary_data.name
        )
        ...
        logger.info(
            f"Updated vocabulary {vocabulary_id} to '{vocabulary_data.name}' for user {user['id']}"
        )
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="Failed to update vocabulary"
        )
...
async def delete_vocabulary_by_name(
    vocabulary_name: str, user: dict = Depends(get_current_user), db: Session = Depends(get_db)
):
    """
    ...
    try:
        ...
        vocabulary = vocab_crud.get_vocabulary_by_name(
            db=db, user_id=user["id"], vocabulary_name=vocabulary_name
        )
