...
async def create_user_tag(
    ...
    try:
        ...
        return {
            ...
            "tag": user_tags_schemas.UserTagResponse.model_validate(
                created_tag
            ).model_dump()
        }
        ...
        raise HTTPException(
            ...
            detail="Failed to create user tag"
        )
...
async def get_user_tag(
    ...
    """
    try:
        tag = crud_service.get_user_tag_secure(
            db=db,
            tag_id=tag_id,
            user_id=user["id"]
        )
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail="Failed to retrieve user tag"
        )
...
async def get_user_tags_by_word_status(
    ...
    try:
        ...
        return {
            ...
            "count": len(tags),
            "tags": [
                user_tags_schemas.UserTagResponse.model_validate(tag).model_dump()
                for tag in tags
            ]
        }
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail="Failed to retrieve user tags"
        )
...
async def update_user_tag(
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail="Failed to update user tag"
        )
...
async def delete_user_tag(
    ...
    """
    try:
        crud_service.delete_user_tag_secure(
            db=db,
            tag_id=tag_id,
            user_id=user["id"]
        )
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail="Failed to delete user tag"
        )