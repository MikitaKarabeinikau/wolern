...
async def get_user_definition(
    definition_id: int, user: dict = Depends(get_current_user), db: Session = Depends(get_db)
):
    """
    ...
    try:
        definition = crud_service.get_user_definition_secure(
            db=db, definition_id=definition_id, user_id=user["id"]
        )
...
async def update_user_definition(
    ...
    try:
        updated_definition = crud_service.update_user_definition_secure(
            ...
            user_id=user["id"],
            part_of_speech=(
                definition_data.part_of_speech.value if definition_data.part_of_speech else None
            ),
            definition=definition_data.definition,
        )
...
async def delete_user_definition(
    ...
    try:
        ...
        return {"success": True, "message": "User definition deleted successfully"}
