...
def verify_user_owns_word_status(
    ...
    try:
        ...
        vocab_word = (
            ...
            .filter(models.VocabularyWords.id == word_status.vocabulary_word_id)
            .first()
        )
        ...
        vocabulary = (
            ...
            .filter(models.Vocabulary.vocabulary_id == vocab_word.vocabulary_id)
            .first()
        )
        ...
        if vocabulary.user_id != user_id:
            ...
            raise OwnershipVerificationError(f"User {user_id} does not own this word status")
...
def verify_user_owns_example(db: Session, example_id: int, user_id: int) -> models.UserExamples:
    """
    ...
    try:
        # Get example
        example = db.query(models.UserExamples).filter(models.UserExamples.id == example_id).first()
...
def verify_user_owns_definition(
    ...
    try:
        definition = (
            ...
            .filter(models.UserDefinitions.id == definition_id)
            .first()
        )
...
def verify_user_owns_synonym(db: Session, synonym_id: int, user_id: int) -> models.UserSynonyms:
    """Verify that a user owns a specific synonym."""
    try:
        synonym = db.query(models.UserSynonyms).filter(models.UserSynonyms.id == synonym_id).first()
...
def verify_user_owns_tag(db: Session, tag_id: int, user_id: int) -> models.UserTags:
    """Verify that a user owns a specific tag."""
    try:
        tag = db.query(models.UserTags).filter(models.UserTags.id == tag_id).first()
...
def verify_user_owns_translation(
    ...
    try:
        translation = (
            ...
            .filter(models.UserTranslations.id == translation_id)
            .first()
        )
