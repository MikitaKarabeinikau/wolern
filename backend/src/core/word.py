...
from core.sound_manager import get_audio_path
from . import utils
import os
import sys
...
from .utils import current_datetime, initial_repeat_time, parse_time_to_str
...
class Word:
    ...
    def set_time_to_repeat(self, minute):
        ...
        logging.info(
            f"Time for repeate for word {self.word.upper()} \\\
                was changed to {self.time_to_repeat} from {last_time}"
        )
    ...
    def down_learning_stage(self):
        ...
        self.learning_stage -= 1
        logging.info(
            f"Learning stage of word {self.word} was decrise by one. Now its: {self.learning_stage}"
        )
    ...
    def change_time_to_repeat(self, minutes):
        ...
        self.time_to_repeat = datetime.now(timezone.utc).strptime(
            self.time_to_repeat, "%d-%m-%Y %H:%M:%S"
