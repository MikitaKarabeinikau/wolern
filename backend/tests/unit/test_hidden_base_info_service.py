...
from backend.src.database.crud.words import add_word, get_all_words_from_db, get_word_id_by_word
...
class TestVocabularyService:
    ...
    def test_add_word_with_full_info(self, db_session):
        ...
        print(all_words)
        assert any(w.word == "cat" for w in all_words), "Word 'cat' was not added to the database."
        ...
        cat_translations = db_session.query(models.Translations).filter(models.Translations.word_id == cat_id).all()
        assert len(cat_translations) == 2, f"Expected 2 translations for 'cat', got {len(cat_translations)}."
        ...
        cat_definitions = db_session.query(models.Definitions).filter(models.Definitions.word_id == cat_id).all()
        assert len(cat_definitions) == 2, f"Expected 2 definitions for 'cat', got {len(cat_definitions)}."
        definition_texts = [d.definition for d in cat_definitions]
        assert "a small domesticated carnivorous mammal" in definition_texts, "Expected definition not found for 'cat'."
        assert "a feline animal" in definition_texts, "Expected definition not found for 'cat'."

        cat_examples = db_session.query(models.Examples).filter(models.Examples.word_id == cat_id).all()
        assert len(cat_examples) == 2, f"Expected 2 examples for 'cat', got {len(cat_examples)}."
        example_texts = [e.example for e in cat_examples]
        print(example_texts)
        assert "The cat sat on the mat." in example_texts, "Expected example not found for 'cat'."
        assert "She has a pet cat." in example_texts, "Expected example not found for 'cat'."
        
        cat_synonyms = db_session.query(models.Synonyms).filter(models.Synonyms.word_id == cat_id).all()
        assert len(cat_synonyms) == 2, f"Expected 2 synonyms for 'cat', got {len(cat_synonyms)}."
        ...
        cat_tags = db_session.query(models.Tags).filter(models.Tags.word_id == cat_id).all()
